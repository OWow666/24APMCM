...
class UNet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        t = t.float() / float(timesteps)  # 使用 timesteps 正则化
...
def perceptual_loss(pred, target):
    vgg_mean = torch.tensor([0.485, 0.456, 0.406], device=pred.device).view(1, 3, 1, 1)
    vgg_std = torch.tensor([0.229, 0.224, 0.225], device=pred.device).view(1, 3, 1, 1)
    pred = (pred + 1) / 2  # 将值从 [-1, 1] 映射到 [0, 1]
    target = (target + 1) / 2
...
def enhance_image(model, diffusion, img_path, output_path):
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(diffusion.timesteps)):
            noisy_img = diffusion.reverse_diffusion(noisy_img, t)

        enhanced_img_tensor = noisy_img.squeeze(0).cpu().clamp(-1, 1)
        enhanced_img = (enhanced_img_tensor + 1) / 2  # 反标准化到[0, 1]
        enhanced_img = enhanced_img.clamp(0, 1)  # 确保在[0, 1]范围内
...
model.load_state_dict(torch.load("./enhanced_ddpm_model.pth"))
